...
export const tauriAPI = {
  ...
  listConversations: async (
    ...
    try {
      return await invoke("list_conversations", { appType });
    } catch (error) {
      console.error("列出对话记录失败:", error);
      throw error;
    }
  },
  ...
  // 删除对话记录
  deleteConversation: async (filePath: string): Promise<void> => {
    try {
      await invoke("delete_conversation", { filePath });
    } catch (error) {
  ...
  // 读取对话内容
  readConversationContent: async (filePath: string): Promise<string> => {
    try {
      return await invoke("read_conversation_content", { filePath });
    } catch (error) {
      console.error("读取对话内容失败:", error);
      throw error;
    }
  },
  ...
  // 读取 Claude 全局规则
  readClaudeRules: async (): Promise<string> => {
    try {
      return await invoke("read_claude_rules");
    } catch (error) {
      console.error("读取 Claude 规则失败:", error);
      throw error;
    }
  },

  // 写入 Claude 全局规则
  writeClaudeRules: async (content: string): Promise<void> => {
    try {
      await invoke("write_claude_rules", { content });
    } catch (error) {
      console.error("写入 Claude 规则失败:", error);
      throw error;
    }
  },

  // 列出 Codex 规则文件
  listCodexRules: async (): Promise<import("../types").CodexRuleFile[]> => {
    try {
      return await invoke("list_codex_rules");
    } catch (error) {
      console.error("列出 Codex 规则失败:", error);
      throw error;
    }
  },

  // 读取 Codex 规则文件
  readCodexRule: async (filename: string): Promise<string> => {
    try {
      return await invoke("read_codex_rule", { filename });
    } catch (error) {
      console.error("读取 Codex 规则失败:", error);
      throw error;
    }
  },

  // 写入 Codex 规则文件
  writeCodexRule: async (
    filename: string,
    content: string,
    tags: string[],
  ): Promise<void> => {
    try {
      await invoke("write_codex_rule", { filename, content, tags });
    } catch (error) {
      console.error("写入 Codex 规则失败:", error);
      throw error;
    }
  },

  // 删除 Codex 规则文件
  deleteCodexRule: async (filename: string): Promise<void> => {
    try {
      await invoke("delete_codex_rule", { filename });
    } catch (error) {
      console.error("删除 Codex 规则失败:", error);
      throw error;
    }
  },
};
